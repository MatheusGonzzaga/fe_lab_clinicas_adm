...
class HomeController with MessageStateMixin {
  ...
  Future<void> startService(int deskNumber) async {
    ...
    switch (result) {
      case Left():
        showError('Erro ao inicar o guichê');
      case Right():
        final resultNextPacient = await _callNextPatientService.execute();
        switch (resultNextPacient) {
          case Left():
          ...
          case Right(value: _):
            asyncstate.AsyncState.hide();
            showInfo('Nenhum paciente aguardando, pode ir tormar um cafézinho');
        }
    }
  }
}
